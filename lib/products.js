...
// Dipakai buat mencocokkan variant_color foto ("Brazil") dengan warna varian
// produk ("brazil", " Brazil ", dst.) tanpa peduli besar/kecil huruf atau spasi.
...
// Dipakai filter pill di halaman etalase, supaya kategori baru yang
// ditambahkan admin (lihat /admin/produk → Kelola Kategori) langsung bisa
// di-browse customer tanpa perlu ubah kode.
...
// Sama seperti getCategories(), tapi dikelompokkan induk → anak — dipakai
// filter di etalase supaya "Sport Authentic" tampil sebagai grup berisi
// "Badminton"/"Running"/"Sepak Bola", bukan pill sejajar yang membingungkan.
// Kategori tanpa anak (mis. 'daily', 'basic', 'custom') tetap tampil sebagai
// pill tunggal seperti sebelumnya (children: []).
export async function getCategoryTree() {
  const flat = await getCategories();
  const bySlug = Object.fromEntries(flat.map((c) => [c.slug, { ...c, children: [] }]));
  const roots = [];
  flat.forEach((c) => {
    if (c.parent_slug && bySlug[c.parent_slug]) {
      bySlug[c.parent_slug].children.push(bySlug[c.slug]);
    } else if (!c.parent_slug) {
      roots.push(bySlug[c.slug]);
    }
  });
  return roots;
}
...
// Satu produk sekarang bisa terdaftar di >1 kategori lewat
// product_category_links (mis. jersey yang sama muncul di "Badminton" DAN
// "Running"). products.category (kolom lama) dipertahankan hanya sebagai
// kategori "utama" untuk breadcrumb/label — sumber kebenaran untuk filter
// adalah junction table ini.
async function productIdsForCategory(supabase, kat) {
  if (!kat) return null;
  const { data, error } = await supabase
    .from('product_category_links')
    .select('product_id, product_categories!inner(slug)')
    .eq('product_categories.slug', kat);
  if (error) {
    console.error('productIdsForCategory failed:', error.message);
    return [];
  }
...
export async function getProductRow(kat, limit = 8, sort = 'newest', search = '', opts = {}) {
  const supabase = getSupabase();
  const categoryProductIds = await productIdsForCategory(supabase, kat);
  // Kategori diminta tapi tidak ada satu produk pun yang link ke situ —
  // jangan lanjut query (kalau dilanjut tanpa filter, .in('id', []) akan
  // balikin SEMUA produk, bukan kosong).
  if (kat && categoryProductIds && categoryProductIds.length === 0) return [];

  let query = supabase
    .from('products')
    .select('id, slug, name, category, gender, price, compare_price, created_at, product_images(url, sort_order)')
    .eq('is_active', true)
    .limit(limit);
...
// Versi dengan pagination sungguhan untuk halaman etalase (bukan cuma
// "limit" seperti getProductRow) — dipakai supaya produk ke-25 dst tetap
// bisa diakses lewat nomor halaman, dan total produk yang ditampilkan
// ("24 produk") mencerminkan jumlah SEBENARNYA di seluruh katalog, bukan
// cuma jumlah yang kebetulan ke-fetch di satu halaman.
export async function getProductPage(kat, page = 1, pageSize = 24, sort = 'newest', search = '', opts = {}) {
  const supabase = getSupabase();
  ...
  const categoryProductIds = await productIdsForCategory(supabase, kat);
  if (kat && categoryProductIds && categoryProductIds.length === 0) {
    return { items: [], total: 0, page: safePage, pageSize, totalPages: 1 };
  }
  ...
  // Sort di level database (bukan di JS) supaya urutan tetap konsisten
  // lintas halaman — sort di JS cuma benar kalau semua data ke-fetch
  // sekaligus, yang justru tidak terjadi begitu ada pagination.
  ...
  if (error) {
    console.error('getProductPage failed:', error.message);
    return { items: [], total: 0, page: safePage, pageSize, totalPages: 1 };
  }
...
// "New Arrivals" beneran, bukan alias ke seluruh etalase: hanya produk yang
// di-input dalam 45 hari terakhir. Kalau katalog baru sekali input semua di
// awal (belum ada yang di-upload dalam 45 hari terakhir), jatuh ke produk
// terbaru yang ada — supaya halaman ini tidak pernah kosong selama
// katalognya sendiri tidak kosong.
const NEW_ARRIVALS_WINDOW_DAYS = 45;

export async function getNewArrivals(limit = 16) {
  const supabase = getSupabase();
  const cutoff = new Date(Date.now() - NEW_ARRIVALS_WINDOW_DAYS * 24 * 60 * 60 * 1000).toISOString();
  const baseQuery = () =>
    supabase
      .from('products')
      .select('id, slug, name, category, gender, price, compare_price, created_at, product_images(url, sort_order)')
  ...
  const { data, error } = await baseQuery().gte('created_at', cutoff);
  if (error) {
    console.error('getNewArrivals failed:', error.message);
    return [];
  }
  if (data && data.length > 0) return data.map(normalizeCard);

  const fallback = await baseQuery();
  if (fallback.error) {
    console.error('getNewArrivals fallback failed:', fallback.error.message);
    return [];
  }
  return (fallback.data || []).map(normalizeCard);
}

// "Bestsellers" beneran: hanya produk yang ditandai admin lewat kolom
// is_bestseller (lihat supabase/add-bestseller-flag.sql) — bukan lagi
// sekadar alias ke etalase penuh. Ditandai manual dari /admin/produk karena
// belum ada data penjualan nyata dari website (lihat catatan di migration
// SQL-nya). Kalau nanti order_items sudah punya cukup data transaksi, fungsi
// ini bisa diganti hitung otomatis dari situ.
export async function getBestsellers(limit = 16) {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from('products')
    .select('id, slug, name, category, gender, price, compare_price, created_at, product_images(url, sort_order)')
...
export async function getProductBySlug(slug) {
  ...
  const { data, error } = await supabase
    .from('products')
    .select(
      'id, slug, name, category, gender, description, material_spec, care_instructions, price, compare_price, product_images(url, sort_order, variant_color), product_variants(id, color, size, stock), product_category_links(product_categories(slug, name))'
    )
    .eq('slug', slug)
    .eq('is_active', true)
    .single();
  ...
  // Semua kategori/slug yang di-link ke produk ini (bisa lebih dari satu,
  // mis. jersey yang sama muncul di "Badminton" dan "Running").
  const categories = (data.product_category_links || [])
    .map((row) => row.product_categories)
    .filter(Boolean);
  ...
  // Foto yang tidak ditandai variant_color (mis. produk lama yang belum
  // pernah ditag, atau memang foto "umum") dipakai sebagai galeri fallback
  // untuk warna manapun yang belum punya foto khususnya sendiri.
  const untaggedGalleryImages = galleryImages
    .filter((img) => !img.variant_color)
    .map((i) => i.url);
  const hasColorTaggedImages = galleryImages.some((img) => img.variant_color);

  // Per warna: pakai foto yang ditag khusus warna itu; kalau belum ada,
  // jatuh ke foto umum; kalau itu pun kosong, jatuh ke galeri lengkap
  // (supaya produk yang belum ditag sama sekali tetap tampil normal).
  const imagesByColor = {};
  for (const color of colors) {
    const key = normalizeColorKey(color);
    const matched = galleryImages
      .filter((img) => normalizeColorKey(img.variant_color) === key)
      .map((i) => i.url);
    imagesByColor[color] = matched.length
      ? matched
      : untaggedGalleryImages.length
      ? untaggedGalleryImages
      : imageUrls;
  }
  ...
  return {
    ...
    images: imageUrls,
    // Foto galeri per warna varian — dipakai halaman produk supaya klik
    // warna (mis. "Brazil") cuma menampilkan foto varian itu saja, bukan
    // seluruh galeri produk. Kosong ({}) kalau produk tidak punya varian
    // warna sama sekali.
    imagesByColor,
    ...
    colors,
    // Daftar varian mentah (id, color, size, stock) — dipakai frontend untuk
    // mencocokkan pilihan warna+ukuran ke satu variant_id yang tepat, dan
    // untuk tahu berapa stok yang tersisa per kombinasi sebelum checkout.
    // Kosong ([]) untuk produk lama yang belum punya varian sama sekali.
